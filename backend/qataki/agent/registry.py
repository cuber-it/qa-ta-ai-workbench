...
Vier Quellen, nach Praefix getrennt:
  pw__*             festverbaute Playwright-Interfaces (mcp_playwright_tools-Lib)
...
from qataki import mcp_client
from mcp_playwright_tools.playwright import tools as _pw
from . import skills

_OBJ = {"type": "object", "properties": {}}

# Kuratiertes Playwright-Kernset. fn = mcp_playwright_tools-Funktion (client, **kwargs)
...
class ToolRegistry:
    ...
    async def tool_specs(self) -> list[ToolDefinition]:
        ...
        for server in cfg.get("servers", {}):
            try:
                for tl in await mcp_client.list_tools(server):
    ...
    async def _dispatch_mcp(self, rest: str, arguments: dict) -> tuple[str, bool]:
        ...
        res = await mcp_client.call_tool(server, tool, arguments)
