...
from qataki.agent import skills, prompts, hooks, validators, loop as L
...
def _patch_base(mp, mode):
    ...
    mp.setattr(L.killswitch, "is_active", lambda: False)
...
async def test_stop_notaus_early(sess_dir, monkeypatch):
    _patch_base(monkeypatch, "final")
    monkeypatch.setattr(L.killswitch, "is_active", lambda: True)
    res = await L.run(S.Session.create(), "x", max_iterations=3)
