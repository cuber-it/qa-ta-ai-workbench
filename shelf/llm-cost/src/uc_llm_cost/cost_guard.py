...
# Injection hooks — the host app wires these (uc_llm_cost.set_config_loader / set_killswitch).
# Defaults keep the package self-contained: no [budget] config, killswitch never active.
_config_loader = lambda: {}      # returns the [budget] section dict
_killswitch = lambda: False      # returns True when execution must hard-stop

log = logging.getLogger(__name__)


def _audit_path() -> Path:
    return _paths.logs("cost_audit.jsonl")
...
def _override_store() -> Path:
    return _paths.data("budget.json")
...
def save_overrides(fields: dict) -> dict:
    ...
    store.write_text(json.dumps(ov, indent=2), "utf-8")
    return ov
