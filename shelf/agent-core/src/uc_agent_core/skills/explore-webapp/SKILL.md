...
Schritte:
1. Mit pw__navigate die Start-URL oeffnen.
2. pw__get_title und pw__current_url pruefen — bist du, wo du sein willst?
3. pw__get_page_content holen und den sichtbaren Inhalt grob einordnen
   (Ueberschriften, Hauptbereiche, Formulare).
4. Interaktive Elemente erfassen: pw__find_by_role fuer button, link, textbox,
   checkbox; pw__get_links fuer die Navigation.
...
pw__screenshot(filename, url)-Aufruf mit url — der navigiert und knipst zusammen.
